                  ...
                  <div class="x_title">
                    <h2>Manage Artists Category</h2>
                      <div class="pull-right">
                          <a href="{{url('admin/artist/list')}}" class="btn btn-primary"><i class="fa fa-plus-circle"></i> Add New</a>
                      </div>
...
$(document).ready(function () {
    $('#users').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('/admin/artist/data') }}",
        columns: [
            {data: 'name', name: 'name'},
            {data: "action",
                render: function (data, type, row) {
                    if (type === 'display') {
                        return '<a class="btn btn-primary btn-xs" href="{{url("admin/artist/category/list/")}}/' + row.type + '"><i class="fa fa-list"></i> Manage Categories</a>';
                    }
                    return data;
                },
